...
formato "YYYY-MM-DD") alle voci di meta_video.json già processate — stesso
principio di estrai_data_completa.py per le foto, serve per la griglia
miniature dell'app Galleria Alexa (raggruppa/ordina i video per giorno
invece che alfabeticamente). ridimensiona_video.py normale scrive già
questo campo per i video NUOVI, ma non ritocca le voci già presenti in
meta_video.json — questo script backfilla quelle vecchie senza ricomprimere
nulla, stesso schema di estrai_giorno_video.py (solo un campo diverso).

Rilegge creation_time dai video ORIGINALI via ffprobe, con lo stesso
fallback sul nome file se il tag manca. Idempotente: salta le voci che
hanno già "data_iso".
"""
import json
import re
...
def main():
    ...
    for i, (nome, v) in enumerate(meta.items(), 1):
        ...
        if not originale:
            ...
            continue
        data_iso = _data_iso_da_creation_time(originale) or _data_iso_da_nome_file(originale.name)
        v["data_iso"] = data_iso
        aggiornate += 1
        if not data_iso:
            senza_data += 1
